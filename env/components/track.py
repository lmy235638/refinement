            ...
            if abs(temp_left_pos - temp_right_pos) < self.config['safety_distance']:
                ...
                if left_vehicle.is_operating or right_vehicle.is_operating:
                    if left_vehicle.is_operating:
                        return_task = right_vehicle.task
                        self.buffer.add_from_allocator(return_task)
                        # 站住不动
                        right_vehicle.task.type = 'temp'
                        right_vehicle.task.start_pos = right_vehicle.pos
                    else:
                        return_task = left_vehicle.task
                        self.buffer.add_from_allocator(return_task)
                        # 站住不动
                ...
                # 2. 一车空闲,一车有任务
                    ...
                    if left_vehicle.task:
                        # 左车有任务撞右车
                        right_vehicle.task = left_vehicle.task
                        right_vehicle.task.type = 'temp'
                        right_vehicle.task.start_pos += self.config['safety_distance']
                    else:
                        # 右车有任务撞左车
                        left_vehicle.task = right_vehicle.task
                        left_vehicle.task.type = 'temp'
                        left_vehicle.task.start_pos -= self.config['safety_distance']
                # 3. 两车任务下发时间相同
                    ...
                    if left_vehicle.load_degree != 0 and right_vehicle.load_degree == 0:
                        ...
                        if left_action == right_action:
                            ...
                            right_vehicle.task.type = 'temp'
                            right_vehicle.task.start_pos = right_vehicle.pos
                            right_vehicle.task.temp_hold_time = 20
                        else:
                        ...
                        if left_action == right_action:
                            ...
                            left_vehicle.task.type = 'temp'
                            left_vehicle.task.start_pos = left_vehicle.pos
                            left_vehicle.task.temp_hold_time = 20
                        else:
                            raise ValueError(f'不是空车追装载车情况出现')
                    elif left_vehicle.load_degree == 0 and right_vehicle.load_degree == 0:
                        if left_action != right_action:
